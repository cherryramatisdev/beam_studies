...
pub type Program<'a> = HashMap<&'a str, usize>;
...
//   Padding4:0..3/unit:8
// >>
fn code_chunk(program: &Program, atoms: &mut Atoms, labels: &mut Labels, module_name: &str) -> Vec<u8> {
...
pub fn compile(file_name: &str, program: Program) -> Vec<u8> {
    ...
    beam.extend(code_chunk(&program, &mut atoms, &mut labels, file_name));
